...
public class battleshipController implements ActionListener{
    ...
    public battleshipController() throws IOException {
        ...
        //Open the server and establish a connection
        establishConnection();
        char[][] oppBoard = server.receiveBoard();
        server.send(oppBoard); //send your array to opponent
        ...
        while(winner) {
            while(turn) {
                ...
                //Probably copying alot from the actionPerformed for readyCannon (or just using it w/ slight modification)
    ...
    public void readyCannons() {
        System.out.println("Inside ready Cannon");
        for(int i = 0; i < 10; i++){
            for(int j = 0; j < 10; j++){
